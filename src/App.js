...
class App extends Component {
  ...
  handleConfirmAddElement = (item) => {
    ...
    fetch('http://localhost:3001/playes',
      {
        method: 'POST',
        headers: {
      ...
      .then(data => {
        sortList(data);
        this.props.client.emit('UPDATE_PLAYERS_LIST', data);
        this.setState({
          players: data
        });
      })
  ...
  handleListElementUpdate = (e, elementId, type) => {
    ...
    switch (type) {
      case globalConst.INCREASE_PLAYER_SCORES:
        ...
        options.body = JSON.stringify(players[playerIndex]);

        fetch('http://localhost:3001/playes', options)
          .then(res => res.json())
          .then(data => {
            sortList(data);
            this.props.client.emit('UPDATE_PLAYERS_LIST', data);
  ...
  logInIcon = () => {
    return (
      <div className="app-system-controls__item" onClick={ this.manageFormVisability }>
  ...
  render() {
    return (
      <div className="full-height">
        { this.state.loginForm ? <LoginForm logIn={ this.logIn } manageFormVisability={this. manageFormVisability } /> : null }
  ...
  componentDidMount() {
    fetch('http://localhost:3001/playes')
      .then(res => res.json())
      .then(data => {
        if (data.players.length) { sortList(data.players) }
        this.setState({
